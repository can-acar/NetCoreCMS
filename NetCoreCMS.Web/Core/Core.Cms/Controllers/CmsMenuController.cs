        ...
        public JsonResult LoadModules(string name)
        {
            ...
            if (!string.IsNullOrEmpty(name))
            {
                ...
                foreach (KeyValuePair<SiteMenu, List<SiteMenuItem>> item in NccMenuHelper.GetModulesSiteMenus())
                {
                    ...
                    {
                        moduleMenuList.Add(new NccModuleMenuViewModel() { ModuleName = subItem.Module,  MenuName = item.Key.Name, Area = subItem.Area, MenuItemName = subItem.Name, MenuUrl = subItem.Url });
                    }
                }
